...
const TableMain = () => {
  ...
  const readyDatasrc = ddd.slice(0, 5)

  // console.log("TableMain config", config);
  // console.log("TableMain readyDatasrc", readyDatasrc);
  // console.log("TableMain pathConfig", pathConfig);
  // console.log("TableMain pathConfig", widgetnemgooReady);
  if (_.isEmpty(pathConfig)) return null
  ...
  // const tdKeys = _.keys(_.head(readyDatasrc));
  ...
  return (
    ...
    >
      <div className="mt-5  flex items-center relative hidden"></div>
      <table className="w-full table-auto border-collapse text-sm ml-0 ">
            ...
            {readyColumn.map((item: any, index: number) => {
              return (
                <th
                  key={item?.id || index}
                  className={`font-bold ${
                    index === readyDatasrc.length ? 'text-center' : 'text-left'
                  } pr-4 break-all`}
                >
                  {item.labelname}
                </th>
        ...
        <tbody className="w-full font-medium">
          {readyDatasrc.map((trItem: any, index: number) => {
              ...
              >
                <td className="whitespace-wrap">
                  <div className=" grid grid-flow-col auto-cols-max gap-3">
                    <div className="w-10 h-10  border   items-center align-middle pt-2 rounded-lg text-center ">
                      <AtomIcon
                        // item={item.icon}
                        item={trItem?.[icon]}
                        checked={false}
