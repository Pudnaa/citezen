...
const SkyResortProfileTable: FC<PropsType> = ({ display = true }) => {
  const {
    config,
    readyDatasrc,
    positionConfig,
  ...
  const [contentQr, setContentQr] = useState<any>();
  ...
  const handlerCloseClickQR = (e: any) => {
    setVisibleQR(false);
  };
  // const statusChangeProcess = (e: any) => {
  //   return (
  //     <>
  //       {" "}
  //       <div className="flex justify-center justify-items-center text-center py-4">
  //         Та <span className="font-bold px-2">{e.ordernumber}</span> захиалгыг
  //         буцаахдаа итгэлтэй байна уу ?
  //       </div>
  //       <div className="flex justify-center gap-0">
  //         <span
  //           className="bg-blue-400 text-white rounded-l-2xl py-1 px-4 cursor-pointer hover:bg-blue-800"
  //           onClick={handlerCloseClick}
  //         >
  //           Үгүй
  //         </span>
  //         <span
  //           className="bg-skyresort text-white rounded-r-2xl py-1 px-4  cursor-pointer hover:bg-yellow-500"
  ...
  const parameters = `&parameters=${JSON.stringify({
    systemMetaGroupId: "1642578675304230",
    customerId: session.id,
  })}`;
  const { data: dataa, error } = useSWR(
    `/api/get-process?processcode=PL_MDVIEW_004${parameters}`
  );
  const data = _.values(dataa);

  //Массивд нэг илүү хоосон элемент үүсээд байсан тул гараар хасав
  if (data[data.length - 1] === "") data.pop();
  //Сүүлд ирсэн нь эхэндээ

  data.sort((x, y) => +new Date(y.orderdate) - +new Date(x.orderdate));
...
export default SkyResortProfileTable;
