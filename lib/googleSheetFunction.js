...
export const callDataComplex = async () => {
  return "";
};
...
const prepareMyWidget = (item) => {
  ...
  for (let j = 0; j < item[0].length; j++) {
    ...
    // console.log(
    //   "tempVAlue - ",
    //   typeof myTempCell.value,
    //   " - ",
    //   myTempCell.value
    // );
    headerRow[j] = myTempCell.value || myTempCell.a1Address;
    // const sw = myTempCell.value?.toLowerCase();
    // const descCase = sw === "desc" || sw === "тайлбар";
    // const imageCase = sw === "image" || sw === "зураг";
    // switch (true) {
    //   case descCase:
    //     headerRow[j] = "position3";
    //     break;
    //   case imageCase:
    //     headerRow[j] = "position2";
    //     break;
    //   default:
