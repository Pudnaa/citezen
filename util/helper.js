...
export function preparePositions(item, positionConfig) {
	...
	_.values(myPositionConfig).map((itemPosition, index) => {
		if (item[_.toLower(itemPosition.fieldpath)]) {
			...
			const positionnemgooReady = replaceTemplate(
				itemPosition.positionnemgooReady,
				item,
			);

			// console.log("itemPosition", itemPosition);
			// console.log("itemPosition.positionnemgoo", itemPosition.positionnemgoo);
	...
	// console.log("XXXXXXXXXXX", newItem);
...
export function prepareDefaults(widgetnemgooReady, router) {
	const widgetDefault = widgetnemgooReady?.listConfig?.default;
	if (_.isEmpty(widgetDefault)) return null;
	...
	const value = myQuery[widgetDefault.urlpath];
	...
	return {
		value: value,
		urlpath: widgetDefault.urlpath,
		topath: widgetDefault.topath,
	};
}
